...
{elseif $mode == "add"}
      ...
      {elseif $def.type == "textarea"}
        ...
        <textarea rows="{$def.rowsize|default:6}" cols="{$def.colsize|default:60}" name="{$header}" {if $def.markup == "xhtml"} id="xhtmlArea"{assign var='xinha_editor' value=true} {else}id="{$header}_{$object->id}"{/if} onKeyUp="textCounter(document.mainform.{$header},document.mainform.{$header}_Len,{if $def.maxsize}{$def.maxsize}{else}{math equation='x * y' x=$def.rowsize y=$def.colsize}{/if});" onChange="getData('index.php?function=validate_field&object={$object->_get_classname()}&field={$header}&value='+DataValueByID('{$header}_{$object->id}'),'{$header}_validation');" {if $validation_messages[$header]}class="validation_failed"{/if} >{$nvp_array[$header]|default:$object->$header}</textarea>
