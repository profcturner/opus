...
class PMSAssessment
{
  ...
  function PMSAssessment($cassessment_id, $assessed_id, $assessor_id)
  {
    // A unique value for how an assessment is used
    $this->cassessment_id = $cassessment_id;
    $this->assessed_id = $assessed_id;
    $this->assessor_id = $assessor_id;

    $this->assessed_name = get_user_name($this->assessed_id);
    // The specific underlying assessment now needs to
    // be fetched.
    ...
    $this->assessment_id = $row[0];

    $this->loadAssessmentRegimeData();
    $this->loadStructure($this->assessment_id);
    $this->loadTotals();
 
  }
  

  function loadStructure($assessment_id)
  {
  ...
  function loadAssessmentRegimeData()
  {
    $sql = "select * from assessmentregime WHERE " .
      "cassessment_id = " . $this->cassessment_id;
    $result = mysql_query($sql)
      or print_mysql_error2("Unable to get assessment data", $sql);
    $this->assessment_regime_data = mysql_fetch_array($result);
    mysql_free_result($result);

    $sql = "select * from assessment WHERE " .
      "assessment_id = " . $this->assessment_regime_data['assessment_id'];
    $result = mysql_query($sql)
      or print_mysql_error2("Unable to get assessment data", $sql);
    $this->assessment_table_data = mysql_fetch_array($result);
    mysql_free_result($result);
