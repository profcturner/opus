...
already present before the \opustext{add} button will present itself.

\subsection{Creating Administrators}
\index{administrators!adding}
...
\index{administrators!institutional}
...
\opustext{Who can help?} link at the bottom of every page, but on other pages
a \opustext{help} link will also take you to the directory.
...
To add a CV Group simply click the \opustext{add} button on this screen. You should
...
Adding an assessment group is as simple as clicking on the \opustext{add} option
...
Once a group has been created, clicking on \opustext{regime} within its listing
...
Clicking on \opustext{add} allows you to choose from the available assessments to
...
When editing a given programme clicking on \opustext{assessment} in the
...
by going to \opusacademicmenu{Advanced -> Activity Types}.
...
by going to \opusacademicmenu{Advanced -> Vacancy Types}.
...
This menu can be found on the \opusacademicmenu{Superuser -> CSV Mapping Menu},
...
\opustip{Don't hijack an existing mapping, delete mappings you don't use and
make one of your own, if you change an existing mapping an upgrade might
...
failing to match the \opustext{Pattern}. This field can be useful if, for 
...
The \opustext{Pattern} might look like so%
...
around and some are missing. So the \opustext{Replacement} field looks as
follows.
...
Adding students is usually done in bulk, via the \opusacademicmenu{Configuration -> Import Data} option,
...
the \opustext{add} button will be shown for adding a new student, simply complete
...
will remain on the menu, until you select another, or \opusacademicmenu{Name -> drop} him or her.

You should also see the student appear in the \opusacademicmenu{Recent} menu at the top,
...
At the top, you will see options to \opustext{cancel} the edit, \opustext{reset the
...
\opustext{manage the applications} made by the student.

\subsubsection{Placement History}

This section shows a summary of placement records for this student, each of
which can be edited. To add a new placement, click on \opustext{manage applications}
...
\opustext{view} to view, or submit the assessment as appropriate. Assessments with marks
...
\opustext{view} a vacancy you will be given an extra option to \opustext{apply with
the selected student} which can be used to make an application on the student's
...
\opustext{Description} shows the name of the CV and where it is stored, the 
\opustext{Valid} column will show either that the CV is valid according to the
...
final column will show if the CV has been \opustext{approved} by a member of the
...
You can undertake a number of options with each CV, to \opustext{view} them, 
\opustext{approve} them or \opustext{revoke} approval. As noted above, approval has
...
\opustext{place} to place the student with one of the applications. Otherwise you
can \opustext{edit} or \opustext{remove} existing applications, which should obviously
...
Finally you can \opustext{drop} a student to remove them from your menu. This is
...
Clicking on the \opustext{edit} link for the staff member in the listing will
...
other way around. However, clicking on \opustext{list students / view home}
...
and other content to users. Each help prompt has a \opustext{lookup} field,
...
through the \opusacademicmenu{Information -> Resources}
...
Some items, particularly companies and students, have a \opustext{Notes} item
...
are filed against this item only in a secondary capacity. The \opustext{Primary}
...
the \opustext{add} button. There are a number of fields given and understanding
...
\opustext{student company}

means student and company contacts have view access to the note.

\opustext{all !student}

means all categories but not students.

\opustip{Legislation may entitle users to be able to obtain access to notes
pertaining to them. So it is important this is remembered when notes are
created.}
...
\opusacademicmenu{Information -> Reports} menu item.
...
The \opustext{Output Format} will provide a choice of output formats, usually
ranging from \opustext{Show Within OPUS} which means the results will be 
...
\opustip{Most of the items on this page are linked, and so, for example, it can
be useful to view which company contacts have logged in, and explore the
company records they represent to ensure all is in order.}
